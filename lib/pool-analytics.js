...
export function getPoolAnalytics(pool) {
    ...
    // Note: In Full Sail, fees go to token holders, not LPs
    const feeApr = tvl > 0 ? (fees24h / tvl) * 365 * 100 : 0;

    // Yield APR from SDK (dinamic_stats.apr)
    // This is the SDK's calculated APR which may include leverage assumptions
    // Note: May differ slightly from Full Sail UI due to internal calculation differences
    const yieldApr = dinamic.apr || 0;
    ...
    // Total external rewards APR (SUI incentives, etc.)
    const externalRewardsApr = externalRewards.reduce((sum, r) => sum + r.apr, 0);

    // oSAIL APR (yield minus external rewards, as estimated)
    const osailApr = Math.max(0, yieldApr - externalRewardsApr);

    // Incentives APR = yield APR (all LP rewards are incentive-based in Full Sail)
    const incentivesApr = yieldApr;
    ...
    return {
        ...
        fullApr: yieldApr,
        fullAprFormatted: formatPercent(yieldApr),
        incentivesApr,
