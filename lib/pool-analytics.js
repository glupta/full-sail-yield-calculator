...
export function getPoolAnalytics(pool) {
    ...
    // Yield APR from SDK (dinamic_stats.apr) - matches Full Sail UI
    // This is the base APR without leverage multiplier assumptions
    const yieldApr = dinamic.apr || 0;

    // Incentives APR is the difference between yield APR and fee APR
    // This includes oSAIL emissions + external rewards (SUI, etc.)
    const incentivesApr = Math.max(0, yieldApr - feeApr);
