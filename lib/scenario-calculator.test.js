    ...
    describe('when pool.rewards has string token format', () => {
        it('should parse token name from module path', () => {
            const scenario = {
                ...
                timeline: 30,
                osailStrategy: 0,
                // No price range set = no leverage applied to external rewards
            };
            const result = calculateScenarioResults(scenario);
            expect(result.externalRewards.length).toBe(1);
            expect(result.externalRewards[0].token).toBe('SUI');
            // External rewards APR is scaled by leverage multiplier (default ~0.38x with no range vs baseline 2.6x)
            // The exact APR depends on leverage calculation
            expect(result.externalRewards[0].apr).toBeGreaterThan(0);
            expect(result.externalRewards[0].projectedValue).toBeGreaterThan(0);
        });
    });
...
describe('calculateScenarioResults - effectiveAPR parameter', () => {
    ...
    describe('when effectiveAPR is provided', () => {
        it('should calculate blended sailAPR from effectiveAPR based on lock strategy', () => {
            const effectiveAPR = 25.5; // 25.5%
            const result = calculateScenarioResults(baseScenario, effectiveAPR);

            // Current logic: redeemAPR = effectiveAPR, lockAPR = 2 * effectiveAPR
            // At 50% lock: sailAPR = 0.5 * (2 * 25.5) + 0.5 * 25.5 = 38.25
            const expectedSailAPR = 0.5 * (effectiveAPR * 2) + 0.5 * effectiveAPR;
            expect(result.sailAPR).toBe(expectedSailAPR);
        });

        it('should calculate lockAPR as 2x effectiveAPR', () => {
            const effectiveAPR = 30;
            const result = calculateScenarioResults(baseScenario, effectiveAPR);

            // Lock APR = 2x base APR (veSAIL has 2x value vs redeemed SAIL)
            expect(result.lockAPR).toBe(effectiveAPR * 2);
        });

        it('should calculate redeemAPR equal to effectiveAPR', () => {
            const effectiveAPR = 30;
            const result = calculateScenarioResults(baseScenario, effectiveAPR);

            // Redeem APR = base APR (effectiveAPR is already the redeem rate)
            expect(result.redeemAPR).toBe(effectiveAPR);
        });

        it('should calculate projected value from APR correctly', () => {
            const effectiveAPR = 36.5; // 36.5% APR (base/redeem rate)
            const result = calculateScenarioResults(baseScenario, effectiveAPR);

            // At 50% lock: lockValue uses 2x APR, redeemValue uses 1x APR
            // Lock portion: 50% * 2 * 36.5% * $10k * (30/365) = ~150
            // Redeem portion: 50% * 1 * 36.5% * $10k * (30/365) = ~75
            // Total: ~225 (but using full year value calculation)
            const timeMultiplier = 30 / 365;
            const lockValue = 0.5 * (effectiveAPR * 2 / 100) * 10000 * timeMultiplier;
            const redeemValue = 0.5 * (effectiveAPR / 100) * 10000 * timeMultiplier;
            const expectedValue = lockValue + redeemValue;

            expect(result.osailValue).toBeCloseTo(expectedValue, 0);
        });
    ...
    describe('edge cases', () => {
        it('should handle very high APR values', () => {
            const result = calculateScenarioResults(baseScenario, 500); // 500% APR base

            expect(result).not.toBeNull();
            // At 50% lock: sailAPR = 0.5 * 1000 + 0.5 * 500 = 750
            expect(result.sailAPR).toBe(750);
        ...
        it('should handle decimal APR values', () => {
            ...
            // At 50% lock: blended = 0.5 * 24.69 + 0.5 * 12.345 = 18.5175
            const expectedSailAPR = 0.5 * (12.345 * 2) + 0.5 * 12.345;
            expect(result.sailAPR).toBe(expectedSailAPR);
        });
